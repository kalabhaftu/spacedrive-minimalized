...
impl LibraryQuery for MediaListingQuery {
	...
	async fn execute(
		...
		// First, find the parent directory entry, falling back to live filesystem for unindexed folders
		let parent_entry = match self.find_parent_directory(db.conn()).await {
			Ok(p) => p,
			Err(e) => {
...
impl MediaListingQuery {
	...
	/// Enumerate live media directly from the filesystem for unindexed directories
	async fn query_live_media(
		&self,
		local_path: &std::path::Path,
		device_slug: &str,
		context: Arc<CoreContext>,
	) -> QueryResult<MediaListingOutput> {
			...
			.map_err(|e| QueryError::Internal(format!("Failed to read directory entry: {}", e)))?
		{
			...
			let symlink_meta = match tokio::fs::symlink_metadata(&child_path).await {
				Ok(m) => m,
				Err(_) => continue,
			};
			...
			if !target_media_types.contains(&content_kind) {
				continue;
			}
		...
		media_files.sort_by(|a, b| b.modified_at.cmp(&a.modified_at));

		let total_count = media_files.len() as u32;
		let has_more = if let Some(limit) = self.input.limit {
			if media_files.len() > limit as usize {
				media_files.truncate(limit as usize);
				true
			} else {
				false
			}

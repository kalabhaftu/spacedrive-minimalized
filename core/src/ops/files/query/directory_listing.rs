	...
	async fn execute(
		...
		match &self.input.path {
			SdPath::Physical { device_slug, path } => {
				...
				// Offline/unmounted fallback: use catalog representation
				match self.find_parent_directory(db.conn()).await {
					Ok(parent_entry) => {
						self.query_indexed_directory_impl(parent_entry.id, db.conn())
							.await
					}
			...
			SdPath::Cloud {
				...
				Err(e) => {
					tracing::debug!("Cloud path not indexed: (err={:?})", e);
					self.query_ephemeral_directory_impl(context, library_id)
						.await
				}
			},
			_ => Err(QueryError::Internal(
				"Unsupported path for directory listing".to_string(),
			)),
		}
	}
}
...
impl DirectoryListingQuery {
	/// Enumerate live directory directly from the filesystem immediately.
	/// Enriches entries with catalog metadata (UUIDs, tags, content identity) if available.
	async fn query_live_directory(
		&self,
		...
		let mut dir = tokio::fs::read_dir(local_path).await.map_err(|e| {
		...
		// Read live entries from the physical filesystem
		let mut fs_entries = Vec::new();
		while let Some(entry) = dir
			.next_entry()
			.await
			.map_err(|e| QueryError::Internal(format!("Failed to read directory entry: {}", e)))?
		{
			let child_path = entry.path();
			if !include_hidden && is_hidden_path(&child_path) {
				continue;
			}

			let symlink_meta = tokio::fs::symlink_metadata(&child_path).await.ok();
			...
			let size = if is_dir {
				...
				symlink_meta.as_ref().map(|m| m.len()).unwrap_or(0)
			};

			let created_at = symlink_meta
				.as_ref()
		...
		// Load catalog records if present in SQLite to enrich entries
		let mut db_files_by_name: HashMap<String, File> = HashMap::new();
		let parent_entry_opt = self.find_parent_directory(db).await.ok();
		if let Some(parent_entry) = &parent_entry_opt {
			if let Ok(indexed_files) = self
				.fetch_indexed_files_for_parent(parent_entry.id, db)
				.await
			{
				for f in indexed_files {
					let key = match &f.extension {
						Some(ext) => format!("{}.{}", f.name, ext),
						None => f.name.clone(),
					};
					db_files_by_name.insert(key, f);
				}
			}
		}
		...
		let mut index_write = global_index.write().await;
		let _ = index_write.ensure_directory(local_path);

		let mut final_files = Vec::with_capacity(fs_entries.len());
		...
		{
			if let Some(mut db_file) = db_files_by_name.remove(&full_name) {
				// Reconcile with live filesystem attributes
				db_file.size = size;
				db_file.modified_at = modified_at;
				db_file.created_at = created_at;
				db_file.accessed_at = accessed_at;
				db_file.kind = kind;
				db_file.sd_path = SdPath::Physical {
					device_slug: device_slug.to_string(),
					path: child_path,
				};
				final_files.push(db_file);
			} else {
				// Uncataloged or newly created file: assign/lookup session UUID in ephemeral cache
				let is_hidden = is_hidden_path(&child_path);
				let entry_uuid = if let Some(uuid) = index_write.get_entry_uuid(&child_path) {
					uuid
				} else {
					...
					let entry_meta = EntryMetadata {
						...
						kind: match kind {
							crate::domain::file::EntryKind::File => StateEntryKind::File,
							crate::domain::file::EntryKind::Directory => StateEntryKind::Directory,
							crate::domain::file::EntryKind::Symlink => StateEntryKind::Symlink,
						},
						size,
						modified: symlink_meta.as_ref().and_then(|m| m.modified().ok()),
						accessed: symlink_meta.as_ref().and_then(|m| m.accessed().ok()),
						created: symlink_meta.as_ref().and_then(|m| m.created().ok()),
						inode: symlink_meta.as_ref().and_then(|m| {
							#[cfg(unix)]
							{
					...
					let _ = index_write.add_entry(child_path.clone(), new_uuid, entry_meta);
					new_uuid
				};
		...
		// Background indexing enhancement: dispatch asynchronously if unindexed without blocking browsing
		if parent_entry_opt.is_none()
			&& !cache.is_indexing(local_path)
			&& !cache.is_indexed(local_path)
		{
			if let Some(library) = context.libraries().await.get_library(library_id).await {
				let ephemeral_index = cache.create_for_indexing(local_path.to_path_buf());
				...
				tokio::spawn(async move {
					let _ = library.jobs().dispatch(indexer_job).await;
				});
			}
		}

		self.sort_files(&mut final_files);

		let total_count = final_files.len() as u32;
		let has_more = if let Some(limit) = self.input.limit {
			if final_files.len() > limit as usize {
				final_files.truncate(limit as usize);
				true
			} else {
				false
			}
	...
	/// Query indexed directory from database (offline/catalog fallback)
	async fn query_indexed_directory_impl(
		&self,
		parent_id: i32,
		db: &DatabaseConnection,
	) -> QueryResult<DirectoryListingOutput> {
		let mut files = self.fetch_indexed_files_for_parent(parent_id, db).await?;
	...
	/// Fetch all indexed entries for a parent directory from the SQLite catalog
	async fn fetch_indexed_files_for_parent(
		&self,
		parent_id: i32,
		db: &DatabaseConnection,
		...
		files.sort_by(|a, b| {
			...
			match self.input.sort_by {
				...
				DirectorySortBy::Type => {
					// Sort by kind (directories first), then extension, then name
					if !folders_first {
						let a_is_dir = matches!(a.kind, EntryKind::Directory);
						let b_is_dir = matches!(b.kind, EntryKind::Directory);
						if a_is_dir != b_is_dir {

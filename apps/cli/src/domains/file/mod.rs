			...
			print_output!(
				...
				|listing: &sd_core::ops::files::query::DirectoryListingOutput| {
					...
					for file in &listing.files {
						let is_directory = file.kind == sd_core::domain::file::EntryKind::Directory;
						let file_type = match file.kind {
							sd_core::domain::file::EntryKind::Directory => "Directory",
							sd_core::domain::file::EntryKind::Symlink => "Symlink",
							sd_core::domain::file::EntryKind::File => "File",
						};
						...
						let display_name = match &file.extension {
							Some(ext) if !ext.is_empty() => format!("{}.{}", file.name, ext),
							_ => file.name.clone(),
						};

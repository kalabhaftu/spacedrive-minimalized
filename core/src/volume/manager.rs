...
impl VolumeManager {
	...
	/// Local-only build: cloud volumes removed, no-op.
	pub async fn load_cloud_volumes_from_db(
		&self,
		_libraries: &[std::sync::Arc<crate::library::Library>],
		_key_manager: std::sync::Arc<crate::crypto::key_manager::KeyManager>,
	) -> VolumeResult<()> {
		Ok(())
	}
	...
	pub async fn resolve_volume_for_sdpath(
		...
		// Local-only mode: cloud paths unsupported
		if sdpath.as_cloud().is_some() {
			Ok(None)
		} else {
